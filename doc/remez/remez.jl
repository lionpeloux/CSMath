...
full_output = true # adjusted by --full
array_format = true # adjusted by --array
preliminary_commands = AbstractString[] # adjusted by --pre
...
function find_extrema(f::Function, grid::Array{BigFloat})
    len = length(grid)
    extrema = Array(Tuple{BigFloat, BigFloat}, 0)
    for i = 1:1:len
...
function winnow_extrema(extrema::Array{Tuple{BigFloat,BigFloat}}, n)
    ...
    best = fill((BigFloat(0), Array(Tuple{BigFloat,BigFloat},0)), n, 2)
...
function test()
    ...
    x, y = goldensection(x->sin(x),parse(BigFloat,"0"), parse(BigFloat,"0.1"), parse(BigFloat,"4"))
    println("  ", x, " -> ", y)
    test(approx_eq(x, asin(parse(BigFloat,"1"))))
    test(approx_eq(y, 1))

    # Test extrema-winnowing algorithm.
    println("Winnow test 1:")
    extrema = [(x, sin(20*x)*sin(197*x))
               for x in parse(BigFloat,"0"):parse(BigFloat,"0.001"):parse(BigFloat,"1")]
...
for arg = ARGS
    if doing_opts && startswith(arg, "-")
        ...
        elseif startswith(arg, "--bits=")
            epsbits = int(arg[length("--bits=")+1:end])
        elseif startswith(arg, "--bigfloatbits=")
            set_bigfloat_precision(int(arg[length("--bigfloatbits=")+1:end]))
